...
<div class="idc-admin-dashboard">
    ...
    tailwind.config = {
      ...
      theme: {
        extend: {
          colors: {
            brand: {
              50:'#eff9ff',100:'#dff3ff',200:'#b9e7ff',300:'#80d5ff',400:'#38bdf8',
              500:'#0ea5e9',600:'#0780b8',700:'#0b5f8a',800:'#0f4363',900:'#122f46'
            }
          },
  ...
  <style>
    .idc-dashboard-container { 
      ...
      background:
        radial-gradient(900px 380px at -10% 10%, rgba(14,165,233,0.06), transparent 14%),
        linear-gradient(180deg,#fbfdff 0%, #f6fbff 40%, #f0f6ff 100%);
      min-height: 100vh;
      color: #1e293b;
    }
    .dark .idc-dashboard-container { 
      background: radial-gradient(900px 380px at -10% 10%, rgba(14,165,233,0.12), transparent 14%), #0b1220; 
      color: #f1f5f9;
    }
  ...
  if (urlParams.get('signup') === 'success') {
    ...
    setTimeout(() => {
      if (typeof alert !== 'undefined') {
        alert('Welcome to SSNYU! Your account has been created successfully. You now have viewer access to the system.');
      }
    }, 1000);
  }
})();
...
function IDCAdminApp(){
  ...
  useEffect(() => {
    ...
    console.log('Setting up broadcast listener for admin dashboard');
    notificationChannel.addEventListener('message', handleBroadcastMessage);
    return () => {
      console.log('Cleaning up broadcast listener for admin dashboard');
  ...
  async function removeRecord(id){
    if(!confirm('Delete record? This will permanently remove it from the database.')) return;
    
    setLoading(true);
    try {
      const numericId = id.replace('c', '');
      await apiCall(`customer/${numericId}`, 'DELETE');
  ...
  async function removeBulk(){
    ...
    if(!confirm(`Delete ${checked.size} selected record(s)? This will permanently remove them from the database.`)) return;
    
    setLoading(true);
    try {
      const idsToDelete = Array.from(checked).map(id => id.replace('c', ''));
      await apiCall('customers/bulk-delete', 'POST', { ids: idsToDelete });
...
<style>
  ...
  * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  /* Bulk */
  const allOnPageChecked = pageItems.length && pageItems.every(x=>checked.has(x.id));
  function toggleAllOnPage(){ const n = new Set(checked); (allOnPageChecked ? pageItems.forEach(x=> n.delete(x.id)) : pageItems.forEach(x=> n.add(x.id))); setChecked(n); }
  function toggleOne(id){ const n=new Set(checked); n.has(id)?n.delete(id):n.add(id); setChecked(n); }

  /* Export */
  function exportCSV(){
    if(!rows.length) return alert('No rows');
    const h=['id','full_name','national_id','passport','country','job_title','created_at','status'];
    const csv = [h.join(',')].concat(rows.map(r=> h.map(k=> `"${(r[k]||'').toString().replace(/"/g,'""')}"`).join(','))).join('\n');
    const blob = new Blob([csv], {type:'text/csv'}); const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href=url; a.download='idc_export.csv'; a.click(); URL.revokeObjectURL(url);
  }
  ...
  return (
    React.createElement('div', { className: 'max-w-[1400px] mx-auto p-3 sm:p-4 md:p-6 space-y-4' },
      ...
      React.createElement('header', { className:'flex items-center gap-2 sm:gap-3 rounded-xl2 shadow-elev bg-white/90 dark:bg-slate-900/70 border border-white/60 dark:border-white/10 backdrop-blur px-2 sm:px-3 md:px-4 py-2' },
        ...
        React.createElement('div', { className:'font-extrabold tracking-tight text-slate-900 dark:text-white' }, 'IDC — Admin Dashboard'),
        ...
        React.createElement('div', { className:'ml-auto flex items-center gap-2' },
          React.createElement('nav', { className:'hidden md:flex items-center gap-2 mr-4' },
            ...
            React.createElement('span', { className:'px-3 py-1.5 rounded-lg bg-brand-100 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 text-sm font-medium' }, 'Admin Dashboard')
          ),
          ...
          React.createElement('div', { className:'flex-1' },
            ...
            React.createElement('div', { className:'mt-6' },
              ...
              React.createElement('div', { className:'flex flex-col gap-2' },
                ...
                React.createElement('button', { className:'px-3 py-2 rounded-lg border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-800', onClick: exportCSV }, 'Export CSV')
              )
            )
          )
        ...
        React.createElement('main', { className:'rounded-xl2 shadow-elev bg-white/90 dark:bg-slate-900/70 border border-white/60 dark:border-white/10 p-4 md:p-6' },
          ...
          React.createElement('div', { className:'flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2' },
            ...
            React.createElement('div', { className:'flex items-center gap-2' },
              React.createElement('button', { className:'px-3 py-1.5 rounded-lg border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40', onClick: removeBulk, disabled: !checked.size }, 'Delete selected'),
                  ...
                  React.createElement('tr', { key:item.id, className:'hover:bg-slate-50 dark:hover:bg-slate-800/40' },
                    ...
                    React.createElement('td', { className:'px-4 py-3' },
                      React.createElement('div',{className:'flex flex-wrap gap-1 justify-end'}, 
                        ...
                        React.createElement('button',{className:'p-2 rounded-md border border-rose-200/70 text-rose-600 hover:bg-rose-50 dark:border-rose-900/40 dark:hover:bg-rose-900/20', onClick:()=> { if(confirm('Delete record?')) removeRecord(item.id); }, title:'Delete' }, Icon.trash({ className:'w-4 h-4' }))
                      )
                    )
                  )
                ) : React.createElement('tr', null,
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const rootElement = document.getElementById('idc-admin-root');
  ...
  function showFallbackContent(element) {
    ...
    element.innerHTML = `
      <div style="padding: 40px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
        <h1 style="color: #1f2937; margin-bottom: 20px;">IDC Admin Dashboard</h1>
